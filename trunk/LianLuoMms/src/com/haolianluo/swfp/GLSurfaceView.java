...
public class GLSurfaceView extends SurfaceView implements SurfaceHolder.Callback {
     ...
     * <p>If this method is
     * called, it must be called before {@link #(Renderer)}
     * is called.
     * <p>
    ...
    class GLThread extends Thread {
        ...
        public void run() {
            ...
            if (LOG_THREADS) {
                HLog.i("GLThread", "starting tid=" + getId());
            }
            try {
                guardedRun();
...
//                HLog.i("22222222222222");
        ...
        private void guardedRun() throws InterruptedException {
                ...
                while (true) {
                    synchronized (sGLThreadManager) {
                        while (true) {
                            ...
                            if (mHaveEgl && mPaused) {
                                if (LOG_SURFACE) {
                                    HLog.i("GLThread", "releasing EGL surface because paused tid=" + getId());
                                }
                                stopEglLocked();
                            }
                            ...
                            if ((! mHasSurface) && (! mWaitingForSurface)) {
                                if (LOG_SURFACE) {
                                    HLog.i("GLThread", "noticed surfaceView surface lost tid=" + getId());
                                }
                                if (mHaveEgl) {
                                    stopEglLocked();
                                }
                            ...
                            if (mHasSurface && mWaitingForSurface) {
                                if (LOG_SURFACE) {
                                    HLog.i("GLThread", "noticed surfaceView surface acquired tid=" + getId());
                                }
                            ...
                            if (LOG_THREADS) {
                                HLog.i("GLThread", "waiting tid=" + getId());
                            }
                    ...
                    if (createEglSurface) {
                        ...
                        if (LOG_RENDERER) {
                            HLog.w("GLThread", "onSurfaceCreated");
                        }
                    ...
                    if (sizeChanged) {
                        if (LOG_RENDERER) {
                            HLog.w("GLThread", "onSurfaceChanged(" + w + ", " + h + ")");
                        }
                    ...
                    if (LOG_RENDERER) {
                        HLog.w("GLThread", "onDrawFrame");
                    }
                    mRenderer.onDrawFrame(gl);
                    if(!mEglHelper.swap()) {
                        if (LOG_SURFACE) {
                            HLog.i("GLThread", "egl surface lost tid=" + getId());
                        }
                    }
            ...
            {
            	Log.d("GLSurfaceView error", "error================================");
            	ex.printStackTrace();
            }
            finally {
                /*
        ...
        public void surfaceCreated() {
            synchronized(sGLThreadManager) {
                if (LOG_THREADS) {
                    HLog.i("GLThread", "surfaceCreated tid=" + getId());
                }
                mHasSurface = true;
                sGLThreadManager.notifyAll();
            }
        ...
        public void surfaceDestroyed() {
            synchronized(sGLThreadManager) {
                if (LOG_THREADS) {
                    HLog.i("GLThread", "surfaceDestroyed tid=" + getId());
                }
                mHasSurface = false;
                sGLThreadManager.notifyAll();
        ...
        public void onWindowResize(int w, int h) {
            synchronized (sGLThreadManager) {
                ...
                while (! mExited && !mPaused && !mRenderComplete ) {
                    if (LOG_SURFACE) {
                        HLog.i("Main thread", "onWindowResize waiting for render complete.");
                    }
                    try {
                        sGLThreadManager.wait();
    ...
    static class LogWriter extends Writer {
        ...
        private void flushBuilder() {
            if (mBuilder.length() > 0) {
                HLog.i("GLSurfaceView", mBuilder.toString());
    ...
    private boolean checkRenderThreadState() {
        if (mGLThread != null) {
            return true;
        } else {
        	return false;
    ...
    private static class GLThreadManager {
        ...
        public synchronized void threadExiting(GLThread thread) {
            if (LOG_THREADS) {
                HLog.i("GLThread", "exiting tid=" +  thread.getId());
            }
            thread.mExited = true;
            if (mEglOwner == thread) {
                mEglOwner = null;
