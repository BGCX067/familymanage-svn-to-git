...
public class PageWidget extends View {
	 ...
	 */
	public void calcCornerXY(float x, float y, int mWidth, int mHeight) {
		if (x <= mWidth / 2)
			mCornerX = 0;
		else
			mCornerX = mWidth;
	...
	public boolean doTouchEvent(MotionEvent event, int mWidth, int mHeight) {
		...
		if (event.getAction() == MotionEvent.ACTION_UP) {
			if (canDragOver(mWidth)) {
				startAnimation(1200, mWidth, mHeight);
			} else {
	...
	private void calcPoints(int mWidth, int mHeight) {
	...
	private void drawNextPageAreaAndShadow(Canvas canvas, Bitmap bitmap, float mMaxLength) {
		mPath1.reset();
	...
	protected void onDraw(Canvas canvas) {
		...
		calcPoints(mWidth, mHeight);
		drawCurrentPageArea(canvas, mCurPageBitmap, mPath0);
		drawNextPageAreaAndShadow(canvas, mNextPageBitmap, mMaxLength);
		drawCurrentPageShadow(canvas, mHeight, mMaxLength);
		drawCurrentBackArea(canvas, mCurPageBitmap, mMaxLength);
...
//		super.onDraw(canvas);
	 ...
	 */
	public void drawCurrentPageShadow(Canvas canvas, int mHeight, float mMaxLength) {
		double degree;
		if (mIsRTandLB) {
			degree = Math.PI
					/ 4
		...
		if(leftx <= 0) {
			leftx = 0;
		} else if(leftx >= mHeight) {
			leftx = mHeight;
		}
		if(rightx <= 0) {
			rightx = 0;
		} else if(rightx >= mHeight) {
			rightx = mHeight;
		}
	 ...
	 */
	private void drawCurrentBackArea(Canvas canvas, Bitmap bitmap, float mMaxLength) {
	...
	private void startAnimation(int delayMillis, int mWidth, int mHeight) {
		int dx, dy;
	...
	public boolean canDragOver(int mWidth) {
