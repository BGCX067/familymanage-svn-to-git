...
public class HSearchAdapter extends BaseAdapter{
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if(convertView == null){
			...
			holder.iv_new = (ImageView) convertView.findViewById(R.id.iv_thread_newsms);
...
//		}
		...
		if(model.thread.type.equals("3")){
			if(model.thread.count != null && Integer.parseInt(model.thread.count) > 1){
				holder.tv_name.setText(drawFlag(appendName,getString(R.string.draft),"("+model.thread.count+")"));
			}else{
				holder.tv_name.setText(drawFlag(appendName,getString(R.string.draft),null));
			}
