...
public class CollectMmsSmsDatabaseHelper extends SQLiteOpenHelper {
    ...
    private static CollectMmsSmsDatabaseHelper mInstance = null;

    static final String DATABASE_NAME = "collectmmssms.db";
    static final int DATABASE_VERSION = 6;

    public CollectMmsSmsDatabaseHelper(Context context) {
     ...
     * database.
     */
    static  CollectMmsSmsDatabaseHelper getInstance(Context context) {
        if (mInstance == null) {
            mInstance = new CollectMmsSmsDatabaseHelper(context);
        }
        return mInstance;
    }
    ...
    public static void updateThread(SQLiteDatabase db, long thread_id) {
        ...
        String query = "SELECT thread_id FROM sms WHERE type=" +
            TelephonyMy.TextBasedSmsColumns.MESSAGE_TYPE_FAILED +
            ...
            try {
                if (smsRows != null) {
                    ...
                    while (smsRows.moveToNext()) {
                        ...
                        cv.put(TelephonyMy.MmsSms.WordsTable.ID, id);
                        cv.put(TelephonyMy.MmsSms.WordsTable.INDEXED_TEXT, body);
                        cv.put(TelephonyMy.MmsSms.WordsTable.SOURCE_ROW_ID, id);
                        cv.put(TelephonyMy.MmsSms.WordsTable.TABLE_ID, 1);
                        db.insert(TABLE_WORDS, TelephonyMy.MmsSms.WordsTable.INDEXED_TEXT, cv);
                    }
                }
            } finally {
                if (smsRows != null) {
