...
public class SmsRejectedReceiver extends BroadcastReceiver {
    ...
    public void onReceive(Context context, Intent intent) {
        if (Settings.Secure.getInt(context.getContentResolver(),
                ...
                TelephonyMy.Sms.Intents.SMS_REJECTED_ACTION.equals(intent.getAction())) {

            int reason = intent.getIntExtra("result", -1);
            boolean outOfMemory = reason == TelephonyMy.Sms.Intents.RESULT_SMS_OUT_OF_MEMORY;
            if (!outOfMemory) {
...
//            Intent viewConvIntent = new Intent(context, ConversationList.class);
            Intent viewConvIntent = new Intent(context, HThreadActivity.class);
