...
public class HThread {
	...
	public HThread getModel(Cursor cursor,int []position){
		...
		//	boolean is1 = getSmsDataForThreadID(date);
			...
			String []arr = {id,name,address,cursor.getString(position[3]),body,date,type,cursor.getString(position[2]),"1"};
			model = setSmsModel(arr,headBp);
		 }else{
			String []arr = {id,name,address,cursor.getString(position[3]),body,cursor.getString(position[1]),type,cursor.getString(position[2]),"0"};
			model = setSmsModel(arr,headBp);
		}
		return model;
	}
...
//			boolean is1 = getSmsDataForThreadID(date);
			...
			String []arr = {thread_id,name,address,cursor.getString(position[3]),body,date, type, cursor.getString(cursor.getColumnIndex(READ)), "1"};
			model = setSmsModel(arr, headBp);
		} else {
			String[] arr = { thread_id, name, address,cursor.getString(position[3]), cursor.getString(position[0]),
					cursor.getString(position[1]), type,cursor.getString(position[2]), "0" };
			model = setSmsModel(arr, headBp);
		}
		return model;
	}
...
//	}
	...
	protected String getString(String address){
		//加飞信好友的时候号码前缀是12520020而不是12520
//		HLog.i("address ===" + address);
//		String[]arrayAddress = address.split(",");
//		
//		for(int i=0;i<arrayAddress.length;i++){
//			int len = arrayAddress[i].length();
//			if(len > 11){
//				if(arrayAddress[i].startsWith("12520") || arrayAddress[i].startsWith("+86")){
//					arrayAddress[i] = arrayAddress[i].substring(len - 11);
//				}
//			}
//		}
//		  if(address.startsWith("12520")){
