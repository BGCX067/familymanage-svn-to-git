...
import com.haolianluo.sms2.ContentRestrictionException;
import com.haolianluo.sms2.dom.smil.SmilParElementImpl;
...
public class SlideModel extends Model implements List<MediaModel>, EventListener {
    ...
    private void internalAdd(MediaModel media) throws IllegalStateException, ContentRestrictionException {
        if (media == null) {
    ...
    private void internalAddOrReplace(MediaModel old, MediaModel media) throws ContentRestrictionException {
        ...
        int addSize = /* (fix bug 629)media.getMediaResizable() ? 0 : */media.getMediaSize();
        int removeSize;
        if (old == null) {
            if (null != mParent) {
     ...
     *
     */
    public boolean add(MediaModel object) throws ContentRestrictionException{
        internalAdd(object);
        notifyModelChanged(true);
        return true;
    }
