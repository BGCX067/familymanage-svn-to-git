...
import android.provider.TelephonyMy.Sms.Conversations;
...
public class Conversation {
    ...
    private static final Uri sAllThreadsUri_collect = Threads.CONTENT_URI_COLLECT.buildUpon().appendQueryParameter("simple", "true").build();
...
//    };
    ...
    private int mUnreadCount;           // jayce add,Number of unread messages.
    private boolean mIsMMS = false;             // jayce add,type of last message.
    ...
    public static Conversation get(Context context, Uri uri, boolean allowQuery) {
        ...
        if (uri.getPathSegments().size() >= 2) {
            try {
                ...
                return get(context, threadId, true);
...
//            mReadContentValues.put("seen", 1);
    ...
    /**
     * jayce add
     * Returns if the last message in this conversation is mms
     * (if it exists).
     */
    public synchronized boolean getIsMMS() {
    	if(HConst.iscollect){
    		return false;
    	}
        return mIsMMS;
    ...
    public static void startQueryForAll(AsyncQueryHandler handler, int token) {
        ...
        if(HConst.iscollect){
        	handler.startQuery(token, null, sAllThreadsUri_collect,ALL_THREADS_PROJECTION, null, null, Conversations.DEFAULT_SORT_ORDER);
        }else{
        	handler.startQuery(token, null, sAllThreadsUri,ALL_THREADS_PROJECTION, null, null, Conversations.DEFAULT_SORT_ORDER);
        }
    }
    ...
    public static void startDelete(AsyncQueryHandler handler, int token, boolean deleteAll,
        ...
        if(HConst.iscollect) {
        	uri = ContentUris.withAppendedId(Threads.CONTENT_COLLECT_URI, threadId);
        }
    ...
    public static void startQueryHaveLockedMessages(AsyncQueryHandler handler, long threadId,
        ...
        if(HConst.iscollect) {
        	uri = Uri.parse("content://com.haolianluo.sms2.collect-mms-sms/locked");
        }
        if (threadId != -1) {
    ...
    private static void fillFromCursor(Context context, Conversation conv,
    	...
    	try {
            synchronized (conv) {
                ...
                int unread = 0;
                Cursor cu = context.getContentResolver().query(Uri.parse("content://sms"), null, 
                		"thread_id = '" + conv.mThreadId + "'" + "AND read = '0'", null, null);
                if(cu != null && cu.getCount() > 0)
                {
                	cu.moveToNext();
                	unread += cu.getCount();
                }
                cu = context.getContentResolver().query(Uri.parse("content://mms"), null, 
                		"thread_id = '" + conv.mThreadId + "'" + "AND read = '0'", null, null);
                if(cu != null && cu.getCount() > 0)
                {
                	cu.moveToNext();
                ...
                {
                	conv.mIsMMS = false;
    	            Cursor cs = context.getContentResolver().query(Uri.parse("content://sms"), null, 
    	            		"thread_id = '" + conv.mThreadId + "'", null, "date DESC");
    	            Cursor cm = context.getContentResolver().query(Uri.parse("content://mms"), null, 
    	            		"thread_id = '" + conv.mThreadId + "'", null, "date DESC");
    	            if(null != cm && cm.getCount() > 0)
    	            {
    	            	...
    	            	else
    	            	{
    		            	...
    		            	String dates = cs.getString(cs.getColumnIndex("date"));
    		            	String datem = cm.getString(cm.getColumnIndex("date"));
    		            	if(dates.length() > datem.length())
    		            	{
    		            		int l = dates.length() - datem.length();
    		            		for(int i = 0; i < l; i++)
    		            		{
    		            			datem = datem + "0";
    		            		}
    		            	}
    		            	else if(datem.length() > dates.length())
    		            	{
    		            		int l = datem.length() - dates.length();
    		            		for(int i = 0; i < l; i++)
    		            		{
    		            			dates = dates + "0";
    		            		}
    		            	}
    		            	long ds = Long.parseLong(dates);
    		            	long dm = Long.parseLong(datem);
    		            	if(ds < dm)
    		            	{
    		            		conv.mIsMMS = true;
    		            	}
...
//    }
    ...
    private static void cacheAllThreads(Context context) {
        ...
        Cursor c = null;
        if(HConst.iscollect){
        	c = context.getContentResolver().query(sAllThreadsUri_collect,
    ...
    private boolean loadFromThreadId(long threadId, boolean allowQuery) {
    	Cursor c = null;
    	if(HConst.iscollect){
    		c = mContext.getContentResolver().query(sAllThreadsUri_collect, ALL_THREADS_PROJECTION,
