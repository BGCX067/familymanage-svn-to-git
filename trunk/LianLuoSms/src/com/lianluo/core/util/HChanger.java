...
public class HChanger {
	...
	public static String ConvertJiaMi(String sSrouce) {
		...
		// System.out.println("length = " + sSrouce.toCharArray().length);
		StringBuffer sb = new StringBuffer();
		for (int i = 0; i < sSrouce.length(); i++) {
			uChar = sSrouce.charAt(i);
			// System.out.println("uChar = " + uChar );
			uHighChar = (short) (uChar / 256);
			uLowChar = (short) (uChar % 256);

			uHighChar++;
			uLowChar++;
			// System.out.println("uHighChar = " + uHighChar +
			// "     uLowChar = " + uLowChar);
			...
			for (int j = 1; j >= 0; j--) {
				...
				bufUnit[j] = bufHigh[j];
				// System.out.println("bufUnit[" + (j + 2) + " ] = " + bufUnit[j
				// + 2] + "     bufUnit[j] = " + bufUnit[j]);
			}
	...
	public static String convertJiemi(String sSource, String sDest) {
		...
		for (int i = 0; i < sSource.length(); i += 4) {
			...
			// System.out.println("uHighChar = " + uHighChar + "    uLowChar = "
			// + uLowChar);
			uHighChar--;
			uLowChar--;
	...
	private static char[] GetHexString(short uHex, char[] sString) {
		...
		nHigh = (short) (uHex / 16);
		nLow = (short) (uHex % 16);
		// System.out.println("nHigh = " + nHigh + "     nLow = " + nLow);
		// char ll = (char)(GetHexChar(nHigh) + GetHexChar(nLow));
		// sString.Format(_L8("%c%c"),GetHexChar(nHigh),GetHexChar(nLow));//将加密之后的短整数合成一个字符
		// for(int j = 3 ; j > 0 ; j --){
		sString[0] = GetHexChar(nHigh);
		sString[1] = GetHexChar(nLow);
		// System.out.println("sString[1] = " + sString[1] +
		// "     sString[0] = " + sString[0]);
		// }
		return sString;
	}
